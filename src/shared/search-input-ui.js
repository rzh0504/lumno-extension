(function(root) {
  ...
  const COMPONENT_CLASSES = Object.freeze({
    root: 'x-lumno-search-input',
    container: 'x-lumno-search-input__container',
    input: 'x-lumno-search-input__field',
    divider: 'x-lumno-search-input__divider',
    icon: 'x-lumno-search-input__icon',
    rightIcon: 'x-lumno-search-input__right-icon'
  });
  const LEGACY_COMPONENT_CLASSES = Object.freeze({
  ...
  function applyBaseStyles(element, className, legacyClassName, declarations, useInlineStyles, useImportantStyles) {
    addClassNames(element, className);
    addClassNames(element, legacyClassName);
    if (className === COMPONENT_CLASSES.container) {
      addClassNames(element, COMPONENT_CLASSES.root);
    }
    if (useInlineStyles) {
  ...
  function appendStylesheet(doc, rootNode, id, href) {
    ...
    if (typeof rootNode.querySelector === 'function') {
      const existingLinks = rootNode.querySelectorAll('link[rel="stylesheet"]');
  ...
  function getSearchInputCssUrl() {
    return getRuntimeUrl('src/shared/search-input.css');
  }
  ...
  function ensureAssetStyles(rootNode, isolatedStyles) {
    ...
    appendStylesheet(document, targetRoot, COMPONENT_STYLE_ID, getSearchInputCssUrl());
  }
  ...
  function ensureComponentStyles(rootNode) {
    ...
    appendStylesheet(document, rootNode, COMPONENT_STYLE_ID, getSearchInputCssUrl());
  }
  ...
  function createSearchInput(options) {
    ...
    const useImportantStyles = Object.prototype.hasOwnProperty.call(config, 'useImportantStyles')
      ...
      : !isolatedStyles;
    const useInlineBaseStyles = Object.prototype.hasOwnProperty.call(config, 'useInlineBaseStyles')
      ? Boolean(config.useInlineBaseStyles)
      : (!isolatedStyles || !styleRoot);
    ...
    applyBaseStyles(input, COMPONENT_CLASSES.input, LEGACY_COMPONENT_CLASSES.input, COMPONENT_STYLES.input, useInlineBaseStyles, useImportantStyles);
    ...
    applyBaseStyles(divider, COMPONENT_CLASSES.divider, LEGACY_COMPONENT_CLASSES.divider, COMPONENT_STYLES.divider, useInlineBaseStyles, useImportantStyles);
    ...
    applyBaseStyles(icon, COMPONENT_CLASSES.icon, LEGACY_COMPONENT_CLASSES.icon, COMPONENT_STYLES.icon, useInlineBaseStyles, useImportantStyles);
    ...
    applyBaseStyles(rightIcon, COMPONENT_CLASSES.rightIcon, LEGACY_COMPONENT_CLASSES.rightIcon, COMPONENT_STYLES.rightIcon, useInlineBaseStyles, useImportantStyles);
    ...
    applyBaseStyles(container, COMPONENT_CLASSES.container, LEGACY_COMPONENT_CLASSES.container, COMPONENT_STYLES.container, useInlineBaseStyles, useImportantStyles);
  ...
  root.LumnoSearchInputUI = api;
  root._x_extension_createSearchInput_2024_unique_ = createSearchInput;
